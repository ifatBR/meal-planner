...
import { LoginSchema, RefreshSchema } from '@app/types/auth'
import {
  loginUser, refreshAccessToken, logoutUser, logoutAllSessions, getAuthenticatedUser,
} from './auth.service'
...
async function authRoutes(fastify: FastifyInstance) {
  fastify.post('/login', async (request, reply) => {
    ...
    const result = await loginUser(
      ...
      body,
      { ipAddress: request.ip ?? null, device: (request.headers['user-agent'] as string) ?? null }
    )
    reply.setCookie(REFRESH_TOKEN_COOKIE, result.refreshToken, refreshCookieOptions(result.ttlDays))
  ...
  fastify.post('/refresh', async (request, reply) => {
    ...
    if (!rawToken) {
      return reply.status(HTTP_STATUS.UNAUTHORIZED).send({ statusCode: HTTP_STATUS.UNAUTHORIZED, code: ERROR_CODES.UNAUTHORIZED, message: ERROR_MESSAGES.UNAUTHORIZED })
    }
    ...
    reply.setCookie(REFRESH_TOKEN_COOKIE, result.refreshToken, refreshCookieOptions(result.ttlDays))
    return { data: { accessToken: result.accessToken, refreshToken: result.refreshToken } }
  })
