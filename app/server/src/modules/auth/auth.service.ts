...
import {
  findUserByEmail, findUserById, findUserWithFirstWorkspace,
  findRefreshTokenByHash, createRefreshToken, revokeRefreshToken, deleteAllUserRefreshTokens,
} from './auth.repository'
import {
  ACCESS_TOKEN_TTL, canUseToken, generateRawRefreshToken,
  getRefreshExpiresAt, getRefreshTtlDays, inferTtlDaysFromRecord, hashRefreshToken,
} from './auth.domain'
...
const unauthorized = () =>
  createError(ERROR_CODES.UNAUTHORIZED, ERROR_MESSAGES.UNAUTHORIZED, HTTP_STATUS.UNAUTHORIZED)()
...
export const loginUser = async (
  ...
  const accessToken = sign(
    { userId: user.id, workspaceId: workspaceUser.workspace_id, role: workspaceUser.role.key as Role },
...
export const refreshAccessToken = async (
  ...
  const accessToken = sign(
    { userId: existing.user_id, workspaceId: workspaceUser.workspace_id, role: workspaceUser.role.key as Role },
    { expiresIn: ACCESS_TOKEN_TTL }
  )
