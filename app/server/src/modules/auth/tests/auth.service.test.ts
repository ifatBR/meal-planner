...
const mockUser = {
  id: 'user-1', email: 'a@test.com', username: 'user1',
  first_name: 'John', last_name: 'Doe', active: true,
  password_hash: 'hash', created_at: new Date(), updated_at: new Date(),
}
const mockWorkspaceUser = {
  workspace_id: 'ws-1', role: { key: 'admin' }, created_at: new Date(),
}
...
describe('loginUser', () => {
  ...
  it('throws 401 if user not found', async () => {
    ...
    await expect(loginUser(prisma, mockSign, { email: 'x@test.com', password: 'pass1234' }, { ipAddress: null, device: null }))
  ...
  it('throws 401 if user is inactive', async () => {
    ...
    await expect(loginUser(prisma, mockSign, { email: 'a@test.com', password: 'pass1234' }, { ipAddress: null, device: null }))
      .rejects.toMatchObject({ statusCode: 401 })
  })
  ...
  it('throws 401 if password is wrong', async () => {
    ...
    await expect(loginUser(prisma, mockSign, { email: 'a@test.com', password: 'wrongpass' }, { ipAddress: null, device: null }))
      .rejects.toMatchObject({ statusCode: 401 })
  })
...
describe('refreshAccessToken', () => {
  ...
  it('rotates the token and returns new access token', async () => {
    vi.mocked(repo.findRefreshTokenByHash).mockResolvedValue({
      id: 'rt-1', user_id: 'user-1', revoked_at: null, expires_at: future,
      created_at: new Date(), ip_address: null, device: null, token_hash: 'h',
    } as any)
    vi.mocked(repo.findUserWithFirstWorkspace).mockResolvedValue(mockWorkspaceUser as any)
  ...
  it('throws 401 for expired token', async () => {
    vi.mocked(repo.findRefreshTokenByHash).mockResolvedValue({
      id: 'rt-1', revoked_at: null, expires_at: new Date(Date.now() - 1000), created_at: new Date(),
    } as any)
    await expect(refreshAccessToken(prisma, mockSign, 'token'))
      .rejects.toMatchObject({ statusCode: 401 })
  })

  it('throws 401 for revoked token', async () => {
    vi.mocked(repo.findRefreshTokenByHash).mockResolvedValue({
      id: 'rt-1', revoked_at: new Date(), expires_at: future, created_at: new Date(),
    } as any)
    await expect(refreshAccessToken(prisma, mockSign, 'token'))
      .rejects.toMatchObject({ statusCode: 401 })
  })
...
describe('logoutUser', () => {
  ...
  it('revokes the session if found and active', async () => {
    vi.mocked(repo.findRefreshTokenByHash).mockResolvedValue({ id: 'rt-1', revoked_at: null } as any)
    vi.mocked(repo.revokeRefreshToken).mockResolvedValue({} as any)
