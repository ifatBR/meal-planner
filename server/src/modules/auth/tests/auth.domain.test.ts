...
describe('isTokenRevoked', () => {
  it('returns true when revokedAt is set', () => expect(isTokenRevoked(new Date())).toBe(true));
  it('returns false when revokedAt is null', () => expect(isTokenRevoked(null)).toBe(false));
});

describe('canUseToken', () => {
  it('returns true for a valid non-revoked token', () =>
    expect(canUseToken({ expires_at: new Date(Date.now() + 60_000), revoked_at: null })).toBe(
      true,
    ));
  ...
  it('returns false for a revoked token', () =>
    expect(canUseToken({ expires_at: new Date(Date.now() + 60_000), revoked_at: new Date() })).toBe(
      false,
    ));
});
...
describe('hashRefreshToken', () => {
  it('is deterministic', () => expect(hashRefreshToken('abc')).toBe(hashRefreshToken('abc')));
  it('produces different hashes for different inputs', () =>
    expect(hashRefreshToken('abc')).not.toBe(hashRefreshToken('xyz')));
  it('returns a 64-char hex string', () => expect(hashRefreshToken('abc')).toHaveLength(64));
});
