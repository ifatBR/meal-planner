...
const mockMealType = { id: 'mt-1', name: 'Breakfast' };

describe('listMealTypes', () => {
  it('returns list of meal types', async () => {
    vi.mocked(repo.listMealTypes).mockResolvedValue([mockMealType]);
...
describe('createMealType', () => {
  it('creates and returns meal type', async () => {
    vi.mocked(repo.createMealType).mockResolvedValue(mockMealType);
    const result = await createMealType(mockPrisma, { name: 'Breakfast' }, 'ws-1');
  ...
  it('throws conflict on P2002', async () => {
    ...
    await expect(createMealType(mockPrisma, { name: 'Breakfast' }, 'ws-1')).rejects.toThrow();
  });
...
describe('updateMealType', () => {
  ...
  it('updates and returns meal type', async () => {
    vi.mocked(repo.getMealTypeById).mockResolvedValue(mockMealType);
    vi.mocked(repo.updateMealType).mockResolvedValue({ id: 'mt-1', name: 'Lunch' });
